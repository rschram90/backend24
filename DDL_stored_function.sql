...
--
-- Create stored_function for smarthomebuddie
--

DELIMITER $$

CREATE FUNCTION CreateOrderFromCart(par_UserNr INT) RETURNS INT 
BEGIN
    ...
    ELSE
        -- Invoegen van de bestellinggegevens in de "orders" tabel
        INSERT INTO orders (UserNr, ShipmentMethodId, PaymentMethodId, ShippingCost, SubTotal, Total, CreatedOn)
        SELECT 
            UserNr, 
            sm.ShipmentMethodId, 
            PaymentMethodId, 
            ...
            NOW()
        FROM carts c
        INNER JOIN shipmentmethods sm ON c.ShipmentMethodId = sm.ShipmentMethodId
        WHERE c.UserNr = par_UserNr;

        -- Ophalen van de ID van de zojuist ingevoegde bestelling
        SELECT LAST_INSERT_ID() INTO created_order_id;

        -- Invoegen van de producten in de bestelling in de "order_products" tabel
        ...
        WHERE cp.UserNr = par_UserNr;

        -- Verwijderen van de inhoud van de winkelwagen van de gebruiker
        DELETE FROM carts WHERE UserNr = par_UserNr;
